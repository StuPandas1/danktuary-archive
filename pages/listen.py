...
from shared import (
    ...
    dank_audio_player,
    authenticate_user,
    convert_local_path_to_drive_relative,
    get_streamable_url_from_relative,
)  # type: ignore
...
if selected_show:
    ...
    if "File Path" not in show_tracks.columns:
        ...
        for _, row in show_tracks.iterrows():
            ...
            # Convert Windows path → OneDrive-relative path
            relative = convert_local_path_to_drive_relative(filepath)
            if not relative:
                continue

            # Get streamable Graph URL
            audio_url = get_streamable_url_from_relative(relative, token)
            if not audio_url:
                continue
